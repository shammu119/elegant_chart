...
- **Subplot margins**: left=0.04, right=0.97, top=0.75, bottom=0.165. Horizontal margins are kept tight because y-tick labels live inside the plot; top/bottom hold the title/subtitle/legend stack and the footer. The top/bottom values and the title (1.234), subtitle (1.220), and legend (1.15) axes-fraction offsets are co-tuned to give the title a ~3pt margin from the figure's top edge and ~1.5pt gaps between title/subtitle/legend/plot, so a **two-line title** plus a **2-row legend** (4 series, `ncol=3`) both fit without clipping or overlapping the plot. The legend's `handlelength` (`_px(1.4)`) and `labelspacing` (`0.1`) are kept short/tight to minimize its vertical footprint, freeing headroom for the title. The footer's `footer_line_y` (0.105) and `caption_y` (0.093) are absolute figure-fraction constants tuned so the x-tick labels land a tight ~2pt above the footer rule (the caption/logo have ~0pt of spare headroom below them at this position). When the baseline relocates to data `y=0` (see Baseline, above), its tick labels move with it, away from `bottom`; the footer is pulled up to match (`footer_line_y = bottom`, `caption_y = bottom - 0.012`), landing ~3.5pt below the relocated labels instead of the much larger gap an unmoved footer would leave.
- **X-axis padding**: left = 0 (the lower xlim sits exactly at the data minimum; the first tick label is left-aligned instead of centered, so it doesn't bleed past the left edge). Right = an auto-measured pad added beyond the data maximum so the last tick label clears the inside y-tick labels on the right edge — override via `x_upper_pad` (relative to the data span), or skip entirely by passing an explicit `xlim`. Y-tick label pad: 0.
- **Edge tick labels & minor ticks**: `align_x_edges` (default `True`) left-aligns the first major x-tick label and right-aligns the last (interior labels stay centered). `x_minor_ticks` adds N evenly-spaced, unlabeled minor ticks between each pair of major ticks (e.g. `1` ⇒ a single midpoint tick, such as mid-year on a yearly axis).
